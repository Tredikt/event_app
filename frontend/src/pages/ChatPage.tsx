            ...
            return (
              ...
              <div className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                {!isMe && (
                  <button
                    onClick={() => partnerId && navigate(`/users/${partnerId}`)}
                    className="flex-shrink-0 w-7 h-7 rounded-full overflow-hidden mb-0.5"
                  >
                    {partnerAvatar
                      ? <img src={partnerAvatar} alt="" className="w-full h-full object-cover" />
                      : <div className="w-full h-full bg-blue-100 flex items-center justify-center text-blue-700 text-xs font-bold">{partnerName[0] ?? '?'}</div>
                    }
                  </button>
                )}
                <div
